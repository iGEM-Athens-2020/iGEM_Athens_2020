function u = iGEM_Athens_2020_Simulation(t,r,M,N,L,W,ub,ke,kr,TR)
...
% ke: engine constant (direction of the linear engine for each bacterium)
% kr: rotation constant (direction of the rotational engine for each 
% bacterium) - if the bacteria do not rotate, use kr = 0
% TR: time values where the linear engine constant changes sign
% 
% 
...
% collisions. If the ODE solver is unsuccessful, try to change the
% collision constant, kc (but always prevent overlap).
kl = 1E-2; % (N*m), Linear springs constant
ka = 1E-15; % (N*m), Angular springs constant
kc = 5.5E-4; % (N*m), Collision constant
Femagn = 80E-12; % Force of engine (N)
